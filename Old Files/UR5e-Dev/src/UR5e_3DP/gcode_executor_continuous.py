...
DEFAULT_VELOCITY     = 0.3    # default velocity scaling (0.0–1.0)
...
class GCodeExecutorContinuous(Node):
    ...
    def start_extrusion(self, total_mm, klipper=None):
        """
        ...
        if klipper is not None:
            klipper.send_extrusion(total_mm, feedrate=KLIPPER_FEEDRATE)

        print(f"  → EXTRUDE {total_mm:.3f} mm (continuous segment)")
...
def main():
    ...
    for i, (seg, traj) in enumerate(zip(segments, planned_segments)):
        ...
        if seg_type == 'extrude':
            ...
            node.start_extrusion(total_extrude, klipper)
