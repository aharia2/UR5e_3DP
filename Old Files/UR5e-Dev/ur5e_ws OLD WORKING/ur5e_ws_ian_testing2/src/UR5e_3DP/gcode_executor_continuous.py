#!/usr/bin/env python3
"""
gcode_executor_continuous.py

Moves the UR5e through waypoints as continuous Cartesian paths.

Key difference from gcode_executor.py
---------------------------------------
Consecutive extrusion waypoints (is_travel=0) are grouped into a single
multi-point GetCartesianPath call, so the robot moves through them as one
smooth continuous path rather than stopping at each point.

Between extrusion segments, travel moves (is_travel=1) are executed as
discrete repositioning moves with no extrusion.

Extrusion is started (via Klipper) just before each continuous extrusion path
and the total filament for the whole segment is sent as one command.

Usage:
    python3 gcode_executor_continuous.py                                    # default waypoints_continuous.csv
    python3 gcode_executor_continuous.py /path/to/file.csv                  # custom CSV
    python3 gcode_executor_continuous.py "/path/to/print.gcode"             # G-code on-the-fly
"""
...
class GCodeExecutorContinuous(Node):
    ...
    def start_extrusion(self, total_mm, feedrate_mm_min, klipper=None):
        """
        Send the extrusion command for a full continuous segment.
        Called just BEFORE execute_trajectory so Klipper extrudes during motion.
        """
        msg = Float64()
        msg.data = total_mm
        ...
        print(f"  → EXTRUDE {total_mm:.3f} mm @ {feedrate_mm_min:.1f} mm/min (continuous segment)")
...
def main():
    ...
    for i, (seg, traj) in enumerate(zip(segments, planned_segments)):
        ...
        if seg_type == 'extrude':
            # Calculate exactly how long the motion will take to match the feedrate
            traj_secs = 0.0
            if traj is not None:
                pts = traj.joint_trajectory.points
                if pts:
            ...
            feedrate_mm_min = KLIPPER_FEEDRATE
            if traj_secs > 0:
                feedrate_mm_min = (total_extrude / traj_secs) * 60.0

            # Send total extrusion for the segment before motion starts
            node.start_extrusion(total_extrude, feedrate_mm_min, klipper)
