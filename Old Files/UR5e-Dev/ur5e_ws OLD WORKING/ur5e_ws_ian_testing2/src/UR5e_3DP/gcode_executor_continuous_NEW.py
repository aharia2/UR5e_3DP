...
Adds velocity-profile-matched extrusion synchronization vs the original.

Key difference from gcode_executor_continuous.py
-------------------------------------------------
Extrusion is no longer sent as a single constant-feedrate command per segment.
Instead:
  1. The planned joint trajectory is time-parameterized by MoveIt.
  2. TCP distances between consecutive trajectory points are estimated from
     joint-velocity magnitude (proxy for TCP speed).
  3. A per-interval extrusion schedule is computed:
       e_i = e_per_mm * dist_i    feedrate_i = e_i / dt_i * 60
  4. A threading.Timer fires each Klipper command at the correct time offset
     from the moment execute_trajectory() is called.
  5. A measured KLIPPER_START_LATENCY offset compensates for the difference
     between Klipper's command-to-motion latency and the robot controller's
     goal-to-motion latency, aligning the two start times.

Usage:
    python3 gcode_executor_continuous_NEW.py                                    # default waypoints_continuous.csv
    python3 gcode_executor_continuous_NEW.py /path/to/file.csv                  # custom CSV
    python3 gcode_executor_continuous_NEW.py "/path/to/print.gcode"             # G-code on-the-fly
"""
...
KLIPPER_START_LATENCY = -0.20  # seconds  (negative = send Klipper command before robot moves)
# ───────────────────────────────────────────────────────────────────────────
...
class GCodeExecutorContinuous(Node):
    ...
    def execute_segment_synchronized(self, seg, traj, klipper):
        """
        Execute a Cartesian trajectory with a single extrusion command timed
        to start KLIPPER_START_LATENCY seconds before the robot moves.

        Sequence (negative latency, e.g. -0.20)
        ----------------------------------------
        1. Compute feedrate so total_extrude finishes in traj_secs.
        2. Send the extrusion command to Klipper.
        3. Sleep |KLIPPER_START_LATENCY| seconds so Klipper spins up.
        4. Call execute_trajectory — robot starts moving.

        Positive latency
        ----------------
        1. Call execute_trajectory — robot starts moving.
        2. Sleep KLIPPER_START_LATENCY seconds.
        3. Send extrusion command.
        (Use positive values only if Klipper is faster than the robot.)
        """
        total_extrude = seg['total_extrude']

        # Compute trajectory duration for feedrate calculation
        ...
        feedrate = (total_extrude / traj_secs) * 60.0 if traj_secs > 0 else KLIPPER_FEEDRATE

        print(f"  [sync] EXTRUDE {total_extrude:.4f} mm @ {feedrate:.1f} mm/min  "
              f"traj={traj_secs:.2f}s  latency={KLIPPER_START_LATENCY:+.3f}s")

        if KLIPPER_START_LATENCY < 0.0:
            # Send extrusion first, then wait before robot starts
            self.start_extrusion(total_extrude, feedrate, klipper)
            _time.sleep(abs(KLIPPER_START_LATENCY))
            ok = self.execute_trajectory(traj)
        else:
            # Start robot first, then send extrusion after delay
            ok = self.execute_trajectory(traj)
            if KLIPPER_START_LATENCY > 0.0:
                _time.sleep(KLIPPER_START_LATENCY)
            self.start_extrusion(total_extrude, feedrate, klipper)
